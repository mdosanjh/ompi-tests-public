...
int main(int argc, char *argv[]) {
    ...
    int dt_size;
    MPI_Type_size(xfer_type,&dt_size);
    ...
    if (0 == myrank) {
        ...
        MPI_Win_create(message, partitions*count, dt_size, info, MPI_COMM_WORLD, &win);
	...
	for (i = 0; i < partitions; ++i) {
            ...
            MPI_Put(message+(partlength*count*i), count, xfer_type, dest, i*count, count, xfer_type, win); 
        }
